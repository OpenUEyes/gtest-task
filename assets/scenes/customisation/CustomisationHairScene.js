class CustomisationHairScene extends Phaser.Scene {
    ...
    constructor() {
        ...
        this.init = function (data) {
            ...
            this.hairFrontIndex = 1;
            this.hairBackIndex = 1;
        }
    }
    ...
    create() {
        ...
        let form = this.add.text(80, game.config.height - 150, 'Select your hair');
        form.setScale(1.5);
        let button = this.add.image(game.config.width / 2, game.config.height - 50, 'button')
            .setInteractive()
            .on(Phaser.Input.Events.GAMEOBJECT_POINTER_DOWN, () => {
                    this.scene.start('CustomisationClothesScene', {
                        body: this.bodyIndex,
        ...
        this.add.image(40, game.config.height / 2, 'vectorLeft')
            ...
            .on(Phaser.Input.Events.GAMEOBJECT_POINTER_DOWN, () => {
                    hairBack.destroy();
                    hairBack = this.add.image(game.config.width / 2, game.config.height / 2, 'hairBack1');
                    hairBack.setScale(.4);
                    this.hairBackIndex = 1;
                    ...
                    hairFront.destroy();
                    hairFront = this.add.image(game.config.width / 2, game.config.height / 2, 'hairFront1');
                    hairFront.setScale(.4);
                    this.hairFrontIndex = 1;

                    form.destroy();
                    form = this.add.text(80, game.config.height - 150, 'Select your hair');
                    form.setScale(1.5);
        ...
        this.add.image(game.config.width - 40, game.config.height / 2, 'vectorRight')
            ...
            .on(Phaser.Input.Events.GAMEOBJECT_POINTER_DOWN, () => {
                    hairBack.destroy();
                    hairBack = this.add.image(game.config.width / 2, game.config.height / 2, 'hairBack2');
                    hairBack.setScale(.4);
                    this.hairBackIndex = 2;
                    ...
                    hairFront.destroy();
                    hairFront = this.add.image(game.config.width / 2, game.config.height / 2, 'hairFront2');
                    hairFront.setScale(.4);
                    this.hairFrontIndex = 2;
